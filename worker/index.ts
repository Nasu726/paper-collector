...
async function loadPapers(db: D1Database): Promise<Paper[]> {
  ...
  // Keep raw recommendation scores inside the Worker. The browser receives only a stable,
  // opaque ordinal rank so it can order the Inbox without presenting pseudo-precise scores.
