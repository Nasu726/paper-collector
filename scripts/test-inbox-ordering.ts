...
assert.deepEqual(rankedOrder.map((candidate) => candidate.id), ['first', 'second', 'third', 'fourth'])
...
assert.deepEqual(firstTieOrder, ['paper-a', 'paper-b'])
...
assert.deepEqual(
  orderInboxPapers(ranked, { first: savedDecision }).map((candidate) => candidate.id),
  ['second', 'third', 'fourth'],
